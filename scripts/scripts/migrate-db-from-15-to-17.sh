...
sudo cp -r $PWD/services/database/data $PWD/services/database/data-15-backup

docker rm -f pg_15_for_migrate

docker run -d --name pg_15_for_migrate \
  -v $PWD/services/database/data:/var/lib/postgresql/data \
  dockerhub.greensight.ru/services/postgis:15-3.4

sleep 5

while ! docker exec -it pg_15_for_migrate pg_isready; do
  sleep 1
done

echo Done

docker exec -it pg_15_for_migrate pg_dumpall -U postgres --clean > backup.sql
